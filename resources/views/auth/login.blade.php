<x-layouts.auth title="Sign In">
    <!-- Session Status -->
    @if(session('status'))
        <div class="mb-4 p-3 rounded-lg bg-success/20 border border-success/30 text-success text-sm text-center">
            {{ session('status') }}
        </div>
    @endif
    ...
    <div class="glass-card rounded-2xl p-8 shadow-glow">
        ...
        <div class="text-center mb-8">
            ...
            <h1 class="text-2xl font-bold text-on-surface mb-2">Cyber Sentinel</h1>
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-5">
            ...
            <div class="flex items-center gap-3">
                <input
                    ...
                    name="remember"
                    type="checkbox"
                    class="w-4 h-4 rounded border-outline-variant/30 text-success focus:ring-success/50 bg-surface-container"
                >
                <label for="remember" class="text-sm text-on-surface-variant select-none cursor-pointer">
        ...
        <div class="relative my-6">
            ...
            <div class="relative flex justify-center text-sm">
                <span class="px-4 bg-transparent text-on-surface-variant">or continue with</span>
            </div>
        </div>

        <!-- Social Login -->
        <div class="grid grid-cols-2 gap-3">
            <button type="button" class="flex items-center justify-center gap-2 px-4 py-2.5 bg-surface-container border border-outline-variant/20 rounded-lg text-sm text-on-surface hover:bg-surface-container-high transition-colors">
